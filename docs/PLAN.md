...
1. 로컬 스캔 품질과 CLI 사용성을 먼저 안정화한다.
2. 리포트 결과를 CI/자동화에서 쓰기 좋게 만든다.
3. remote GitHub scan은 명시적 옵션으로 설계한 뒤 구현한다.
...
### 1. Finding/score 정책 문서화

- 작업: severity 기준, category weight, finding ID 안정성 정책을 문서화한다.
- 배경: rule이 늘어날수록 감점 기준이 흔들리면 RepoTrust 자체 신뢰도가 떨어진다.
- 완료 기준:
  - severity별 의미와 예시가 문서화된다.
  - 현재 category weight와 변경 원칙이 문서화된다.
  - finding ID를 public-ish contract로 다루는 기준이 정리된다.
- 영향 범위: `docs/domain-context.md`, `docs/trd.md`, 필요 시 `docs/adr.md`.
- 검증: `.venv/bin/python -m pytest -q`

### 2. CLI UX 정리

- 작업: CLI help, error message, summary 출력, `--format` 검증 UX를 점검하고 개선한다.
- 배경: 사용자가 처음 실행했을 때 스캔 결과와 실패 이유를 바로 이해해야 한다.
- 완료 기준:
  - `repotrust --help`, `repotrust scan --help` 출력이 명확하다.
  - invalid format과 missing target 동작이 테스트된다.
  - JSON stdout과 stderr summary 분리가 유지된다.
- 영향 범위: `src/repotrust/cli.py`, `tests/test_cli.py`, 필요 시 `README.md`.
- 검증: `.venv/bin/python -m pytest -q`
...
1. `Finding/score 정책 문서화`부터 시작한다.
