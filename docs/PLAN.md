...
- 작업: `repotrust.toml` 기반 설정 파일을 명시적 `--config <path>` 옵션으로 읽어 적용한다.
- 배경: 설계에서 v1 config는 `policy.fail_under`와 네 개 category weight로 제한하기로 결정했다.
- 완료 기준:
  - `--config <path>` 옵션이 추가된다.
  - `policy.fail_under`가 `--fail-under` 기본값으로 동작하며 CLI flag가 우선한다.
  - `[weights]` 설정이 score 계산에 반영된다.
  - invalid config는 명확한 usage error로 실패한다.
  - Python 3.10 호환 TOML parser fallback이 처리된다.
- 영향 범위: `src/repotrust/`, `tests/`, `pyproject.toml`, `docs/trd.md`, `docs/testing-and-validation.md`.
