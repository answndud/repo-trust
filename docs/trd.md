...
JSON report는 외부 도구와 CI에서 사용할 수 있는 안정적인 contract로 취급한다. 현재 contract는 `schema_version: "1.1"`을 최상위에 포함한다.
...
- `assessment`: verdict, confidence, coverage, summary, reasons, next_actions.
...
- `schema_version` remains `"1.1"` while changes only add new stable finding IDs or renderer behavior without changing top-level JSON keys.
