...
- `docs/finding-reference.md`: stable finding ID catalog와 CI policy 예외 기준.
- `docs/json-report-reference.md`: JSON report schema `1.2` key contract와 parsing 예시.
...
- active 작업이 모두 끝났고 프로젝트가 아직 완성 전이면 `docs/PLAN.md`에 다음 개발 계획을 우선순위 순서로 추가한다.
- CLI v1 완성 전까지 `PLAN.md` → 구현 → 테스트 → 자체 코드리뷰 → `COMPLETED.md` archive 흐름을 반복한다.
- post-v1 반복 작업은 작은 story 하나를 `In Progress`로 승격하고, 구현/검증/리뷰/커밋/archive 후 다음 story로 넘어간다.
- 작업 완료 전 `git diff --stat`과 변경 diff를 검토해 회귀, 누락 테스트, 문서 drift를 확인한다.
