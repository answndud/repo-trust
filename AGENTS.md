...
- `docs/design/`: PRD/TRD/ADR, architecture, domain context 같은 설계 문서.
- `docs/reference/`: finding ID와 JSON report contract 같은 안정적 참조 문서.
- `docs/process/`: 개발 workflow와 검증 절차 문서.

`docs/` 루트에는 `PLAN.md`, `PROGRESS.md`, `COMPLETED.md`만 둔다. 다른 문서는 성격에 맞는 하위 폴더에 둔다.
