...
기존 개발용 명령인 `repotrust scan TARGET`도 계속 동작합니다. 새 문서에서는 사용자가 바로 이해하기 쉬운 `repo-trust TARGET` 형태를 기본으로 설명합니다.
...
```bash
repo-trust "https://github.com/openai/codex" --format json
```
...
```bash
repo-trust "https://github.com/openai/codex" --remote --format json
```
...
```bash
repo-trust tests/fixtures/repos/good-python --format markdown
repo-trust tests/fixtures/repos/good-python --format json --output /tmp/repotrust-good.json
...
```bash
repo-trust tests/fixtures/repos/risky-install --format html --output /tmp/repotrust-risky.html
```
...
source /tmp/repotrust-clean/.venv/bin/activate
python -m pip install -e '.[dev]'
repo-trust --version
python -m pytest -q
```
