...
- 결과는 터미널 대시보드, JSON, static HTML, 저장된 JSON 간 터미널 비교로 볼 수 있습니다.
...
`compare`는 terminal text만 출력합니다. 공유용 산출물이 필요하면 원본 JSON 리포트와 HTML 리포트를 함께 보관하세요.
...
- `Persisting findings`는 아직 남아 있는 문제입니다. JSON/HTML 리포트에서 해당 finding ID를 다시 확인하세요.
