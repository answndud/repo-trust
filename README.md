...
처음 사용하는 사람은 `repo-trust-kr`로 한국어 Console Mode를 열어 workflow를 익히면 됩니다. 익숙해진 뒤에는 `repo-trust html`, `repo-trust json`, `repo-trust check`를 직접 입력하면 반복 실행과 자동화가 쉽습니다. `--help`를 붙이면 영어와 한국어 중 원하는 도움말을 선택할 수 있습니다. 공개 피드백을 받을 때는 먼저 자기 저장소나 이미 아는 프로젝트를 로컬로 검사해 결과가 기대와 맞는지 확인해 보세요.
...
## 4. Command Mode: GitHub URL을 HTML로 저장
...
## 5. Command Mode: GitHub URL을 JSON으로 저장
...
## 6. Command Mode: 로컬 폴더를 HTML로 저장
