...
curl -L -o repotrust-v0.2.1.tar.gz https://github.com/answndud/repo-trust/archive/refs/tags/v0.2.1.tar.gz
python -m pip install repotrust-v0.2.1.tar.gz
repo-trust-kr
```

터미널을 새로 열었다면 실행 전에 `source .venv/bin/activate`로 가상환경을 다시 활성화하세요.
개발 작업을 할 때는 저장소를 clone한 뒤 `python -m pip install -e '.[dev]'`를 사용하세요. PyPI 배포는 현재 프로젝트 범위가 아니며, 공식 배포 채널은 GitHub Releases입니다.
